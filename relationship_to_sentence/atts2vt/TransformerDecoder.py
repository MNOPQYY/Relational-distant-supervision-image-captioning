...
class XECriterion(nn.Module):
    ...
    def forward(self, pred, target, lengths, weights):
        max_len = max(lengths)
        ...
        loss = torch.sum(loss *weights) / torch.sum(mask)
...
class LabelSmoothingCriterion(nn.Module):
    ...
    def forward(self, pred, target, lengths, weights):
        ...
        loss = (self.criterion(pred, true_dist.detach()).sum(1) * mask*weights).sum() / mask.sum()
        return loss

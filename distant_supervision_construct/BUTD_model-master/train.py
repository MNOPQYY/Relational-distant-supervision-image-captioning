...
def train():
    ...
    for split, caps in captions.items():
        ...
        for fn in tqdm.tqdm(list(caps.keys()),ncols=100):
            seqs= caps[fn]
            tmp = []
            for seq_dict in seqs:
                seq = seq_dict['caption']
                data_from = seq_dict['from']
                tmp.append({'caption':[decoder.sos_id] +
                           [word2idx.get(w, None) or word2idx['<UNK>'] for w in seq] +
                           [decoder.eos_id],'from':data_from})
    ...
    for fn in captions['val']:
        val_captions[fn] = [[]]
    val_data = get_dataloader(opt.fc_feats, opt.att_feats, val_captions, decoder.pad_id,
    ...
    def forward(data, training=True, ss_prob=0.0):
        ...
        loss_val = 0.0
        reward_val = 0.0
        for fns, fc_feats, att_feats, (caps_tensor, lengths), ground_truth, weights_tensor in tqdm.tqdm(data, ncols=100):
            fc_feats = fc_feats.to(opt.device)
            att_feats = att_feats.to(opt.device)
            caps_tensor = caps_tensor.to(opt.device)
            weights_tensor = weights_tensor.to(opt.device)
            ...
            if training and train_mode == 'rl':
                ...
                loss = xe_criterion(pred, caps_tensor[:, 1:], lengths, weights_tensor[:, 1:])
